...
public class MainActivity extends AppCompatActivity implements CountryFragment.Mylistener,
    ...
    public void helperinit(){
        ...
        helpwordarr[2] = "这里是记忆曲线，它会记录那些完全陌生的知识点是怎样被记忆的，也就是新知识点"+
                ...
                "的情况。";
        helpwordarr[3] = "这里是比较复杂的一个地方，可以对记忆计划进行修改，每个知识点都有" +
                "四种不同的熟悉程度。\n每种程度都有5个阶段，对应了一个时间。没错，这个时间就是" +
                "复习的间隔时间，比如陌生知识点第一阶段15秒就需要复习一次，而非常熟练时一个月可能" +
                "只需要复习一次。\n这些都是可以根据自己的计划和记忆情况自己设定的！以后还会开发出自动" +
                "调整的功能（又挖了一个大坑orz）";
        helpwordarr[4] = "这里是背诵任务界面，会给出问题，然后下面对应了4个不同选项。\n大体可以这样分:"+
                "陌生->完全不知道；有印象->知道一点但是并不会；比较熟练->会但是感觉记忆不了太长时间"+
                "；非常熟练->很久都忘不了的知识。\n问题也会有一些信息，会给出上次记这个知识点距离现在有多久,以及"+
                "记忆它的次数，还有记忆等级（每5级对应4个不同的记忆阶段）。这个系统会根据你的选项来调整记忆等级。";
